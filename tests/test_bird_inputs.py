...
from main import SimulationConfig, normalize_inputs


class BirdInputTests(unittest.TestCase):
    def test_normalize_inputs_are_centered_and_normalized(self) -> None:
    ...
    def test_normalize_inputs_clips_distance_velocity_and_offsets(self) -> None:
        config = SimulationConfig(world_width=500.0, world_height=800.0, velocity_min=-12.0, velocity_max=12.0)
        ...
        inputs = normalize_inputs(bird, [far_pipe], config)
    ...
    def test_normalize_inputs_without_pipe_returns_neutral_defaults(self) -> None:
        config = SimulationConfig(world_width=500.0, world_height=800.0, velocity_min=-12.0, velocity_max=12.0)
        bird = Bird(y=100.0, velocity=20.0, x=100.0, world_height=800.0)

        inputs = normalize_inputs(bird, [], config)

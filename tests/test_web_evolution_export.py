    ...
    def test_export_schema_contains_generation_pipe_seed_and_top_k_genomes(self) -> None:
        ...
        for generation in payload["generations"]:
            ...
            for genome in generation["genomes"]:
                self.assertIn("species_id", genome)
                self.assertIsInstance(genome["species_id"], int)
                self.assertIn("rank", genome)
                self.assertIn("fitness", genome)
                self.assertIn("pipes_passed_mean", genome)
                self.assertIn("pipes_passed_max", genome)
                self.assertIn("pipes_mean", genome)
                self.assertIn("pipes_max", genome)
